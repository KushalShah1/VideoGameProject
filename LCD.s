...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
		LDR R1,=SSI0_SR_R
LOOP	LDR R2,[R1]
		AND R2,#0X010		;Read SSI0_SR_R and check bit 4,
		CMP R2,#0X010		;If bit 4 is high
		BEQ LOOP			;loop back to step 1
		LDR R2,=DC
		MOV R3,#DC_COMMAND		;Clear D/C=PA6 to zero
		STR R3,[R2]
		LDR R2,=SSI0_DR_R
		STR R0,[R2]				;Write the command to SSI0_DR_R
		LDR R1,=SSI0_SR_R
LOOP1	LDR R2,[R1]
		AND R2,#0X010			;Read SSI0_SR_R and check bit 4
		CMP R2,#0X010			;If bit 4 is high
		BEQ LOOP1				;loop back to step 5
...
;4) Write the 8-bit data to SSI0_DR_R
		LDR R1,=SSI0_SR_R	;read SSI0_SR_R
LOOP2	LDR R2,[R1]
		AND R2,#0X01		;CHECK BIT 4
		CMP R2,#0X01
		BNE LOOP2			;LOOP BACK IF LOW
    	LDR R2,=DC
		MOV R3,#DC_DATA		;SET D/C TO ONE
		STR R3,[R2]
		LDR R2,=SSI0_DR_R	;WRITE TO SSI0_DR_R
		STR R0,[R2]
    BX  LR                          ;   return
